...
for pline in pufd:
  ...
  leftFlank = str( contigSeq[startPos:pos-1].upper() )
  rightFlank = str( contigSeq[pos+rightFlankOffset:endPos].upper() )
  refSNP = str( contigSeq[pos-1:pos].upper() )
  #ofile.write( contigName+"."+str(snpNumber).zfill(7)"\n" )
  #ofile.write( contigName+"."+str(pos).zfill(7)+"\t" ) # padded position number
  ofile.write( contigName+"_"+str(pos)+"\t" )
  ofile.write( leftFlank+snpType+rightFlank+"\t"+refSNP+"\n" )
